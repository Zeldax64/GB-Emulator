/*
	Cinoop - cpu.c file
	Source: https://github.com/CTurt/Cinoop/blob/master/source/cpu.c
...
GBE source
Which dbinstructions modify flags:
...
const struct instruction dbinstructions[256] = {
	{ "NOP", 0, nop },                           // 0x00
...
void cpuStep(void) {
	...
	if(dbinstructions[instruction].operandLength == 1) operand = (unsigned short)readByte(registers.pc);
	if(dbinstructions[instruction].operandLength == 2) operand = readShort(registers.pc);
	registers.pc += dbinstructions[instruction].operandLength;
	
	//if(dbinstructions[instruction].operandLength) printf(dbinstructions[instruction].disassembly, operand);
	//else printf(dbinstructions[instruction].disassembly);
	//printf("\n");
	
	switch(dbinstructions[instruction].operandLength) {
		case 0:
			((void (*)(void))dbinstructions[instruction].execute)();
			break;
		
		case 1:
			((void (*)(unsigned char))dbinstructions[instruction].execute)((unsigned char)operand);
			break;
		
		case 2:
			((void (*)(unsigned short))dbinstructions[instruction].execute)(operand);
			break;
	}
...
// 0x98
void sbc_b(void) { sbc(registers.b); }

// 0x99
void sbc_c(void) { sbc(registers.c); }

// 0x9a
void sbc_d(void) { sbc(registers.d); }

// 0x9b
void sbc_e(void) { sbc(registers.e); }

// 0x9c
void sbc_h(void) { sbc(registers.h); }

// 0x9d
void sbc_l(void) { sbc(registers.l); }

// 0x9e
void sbc_hlp(void) { sbc(readByte(registers.hl)); }

// 0x9f
void sbc_a(void) { sbc(registers.a); }
